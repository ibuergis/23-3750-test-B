...
function authenticate(request, response) {
    ...
    let hash = crypto.createHash('sha256').update(request.body.password).digest('hex');
    if (hash === correctUser.password) {
        // generates a salt to make a unique hash for the cookie, this is done to avoid a stolen cookie to be working forever.
        // the hex will be regenerated everytime it checks for authentication. Reason is that on a password change all cookies
        // get invalidated
